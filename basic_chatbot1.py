...
def fetch_dynamic_data():
    try:
        ...
        logging.info("Dynamic data updated")
    except Exception as e:
...
def find_answer(user_message, session_id):
    ...
    if corrected in exact_matches:
        ...
        if matched_category == "food" or any(k in query_clean for k in ["food", "canteen", "mess", "store"]):
            ...
            if "canteen" in query_clean and ("location" in query_clean or "where" in query_clean):
                ...
                query = "What are the canteen facilities at RVR & JC College"
            logging.debug(f"Food query mapped to: {query}")

        # Library-specific queries
        elif matched_category == "library" or "library" in query_clean:
            logging.debug(f"Processing library query: {query_clean}")
            ...
            logging.debug(f"Library query mapped to: {query}")

        # Hostel-specific queries
        elif matched_category == "hostel" or "hostel" in query_clean:
            logging.debug(f"Processing hostel query: {query_clean}")
        ...
        # Fee-specific queries
        elif matched_category == "fees" or "fees" in query_clean:
            logging.debug(f"Processing fee query: {query_clean}")
            ...
            logging.debug(f"Fee query mapped to: {query}")

        # Placement-specific queries
        elif matched_category in ["placements", "placement_head", "placement_cell_head", "companies_visited"]:
            response = handle_placement_query(query_clean, matched_category, year, department)
            logging.debug(f"Returning placement response: {response}")
            response = f"{response} 💼"
            return f"Glad you're excited! {response}" if sentiment > 0.4 else response

        # Department-specific queries
        elif matched_category == "departments" or matched_category == "hod":
            logging.debug(f"Processing department query: {query_clean}")
    ...
    if "i mean" in query_clean and session_context[session_id][-2:]:
        ...
        if matched_category == "placement_cell_head" or ("head" in prev_query and "placement" in prev_query):
            ...
            logging.debug(f"Processing follow-up food query: {prev_query}")
            if "canteen" in prev_query and ("location" in prev_query or "where" in prev_query):
                query = "Where is the canteen located at RVR & JC College"
            elif "store" in prev_query and ("location" in prev_query or "where" in prev_query):
                query = "Where is the campus store located at RVR & JC College"
            elif "canteen" in prev_query and ("food" in prev_query or "items" in prev_query or "menu" in prev_query):
                query = "What food items are available in the canteen at RVR & JC College"
            elif "store" in prev_query and ("food" in prev_query or "items" in prev_query or "menu" in prev_query):
                query = "What food items are available in the campus store at RVR & JC College"
            elif "canteen" in prev_query:
                query = "What are the canteen facilities at RVR & JC College"
            elif "store" in prev_query or "shop" in prev_query:
                query = "What is the campus store at RVR & JC College"
            elif "hostel" in prev_query or "mess" in prev_query or "menu" in prev_query:
                query = "What types of food are provided in the hostels at RVR & JC College"
            else:
                query = "What are the canteen facilities at RVR & JC College"
            logging.debug(f"Follow-up food query mapped to: {query}")
        elif matched_category == "library":
            logging.debug(f"Processing follow-up library query: {prev_query}")
            if "location" in prev_query or "where" in prev_query or "address" in prev_query:
                query = "Where is the library located at RVR & JC College"
            elif any(k in prev_query for k in ["number", "how many", "volumes", "titles"]):
                query = "What is the number of books in the library at RVR & JC College"
            elif any(k in prev_query for k in ["return", "duration", "how long"]):
                query = "What is the duration to return books to the library at RVR & JC College"
            elif "timing" in prev_query or "timings" in prev_query or "hours" in prev_query:
                query = "What are the timings of the library at RVR & JC College"
            elif any(k in prev_query for k in ["charges", "penalty", "cost", "fine"]):
                query = "What are the charges or penalties for the library at RVR & JC College"
            elif any(k in prev_query for k in ["types", "type", "journals"]) and not any(k in prev_query for k in ["return", "duration"]):
                query = "What types of books are available in the library at RVR & JC College"
            else:
                query = "What are the library facilities at RVR & JC College"
            logging.debug(f"Follow-up library query mapped to: {query}")
        elif matched_category == "hostel":
            logging.debug(f"Processing follow-up hostel query: {prev_query}")
            if "availability" in prev_query or "available" in prev_query or ("boys" in prev_query and "girls" in prev_query):
                query = "What is the availability of hostels for boys and girls at RVR & JC College"
            elif "location" in prev_query or "where" in prev_query or "address" in prev_query:
                if "boys" in prev_query or "boys hostel" in prev_query:
                    query = "Where is the boys' hostel located at RVR & JC College"
                elif any(k in prev_query for k in ["girls", "girl", "girls'", "girl's", "womens", "women's hostel"]):
                    query = "Where is the girls' hostel located at RVR & JC College"
                else:
                    query = "What are the hostel facilities at RVR & JC College"
            elif "fees" in prev_query or "cost" in prev_query or "price" in prev_query or "hostel fee" in prev_query:
                if "boys" in prev_query or "hostel fee for boys" in prev_query:
                    query = "What is the hostel fee for boys at RVR & JC College"
                elif any(k in prev_query for k in ["girls", "girl", "girls'", "girl's", "womens", "women's hostel", "hostel fee for girls"]):
                    query = "What is the hostel fee for girls at RVR & JC College"
                else:
                    query = "What are the hostel fees at RVR & JC College"
            ...
            if "department-wise tuition fee" in prev_query or "management fee" in prev_query or any(k in prev_query for k in ["btech fee", "mtech fee", "mca fee", "mba fee", "department wise fee"]):
                ...
                query = "What is the fee structure for B.Tech programs at RVR & JC College"
            logging.debug(f"Follow-up fee query mapped to: {query}")
        elif matched_category == "departments" or matched_category == "hod":
            if "hod" in prev_query or "head of department" in prev_query:
                if "cse" in prev_query and not ("csd" in prev_query or "data science" in prev_query):
                    query = "Who is the HOD of the Computer Science & Engineering department at RVR & JC College"
                elif "csd" in prev_query or "data science" in prev_query:
                    query = "Who is the HOD of the Computer Science & Engineering (Data Science) department at RVR & JC College"
                elif "csbs" in prev_query or "machine learning" in prev_query:
                    query = "Who is the HOD of the Computer Science and Machine Learning department at RVR & JC College"
                elif "chemical" in prev_query:
                    query = "Who is the HOD of the Chemical Engineering department at RVR & JC College"
                elif "civil" in prev_query:
                    query = "Who is the HOD of the Civil Engineering department at RVR & JC College"
                elif "ece" in prev_query or "electronics" in prev_query:
                    query = "Who is the HOD of the Electronics & Communication Engineering department at RVR & JC College"
                elif "eee" in prev_query or "electrical" in prev_query:
                    query = "Who is the HOD of the Electrical & Electronics Engineering department at RVR & JC College"
                elif "it" in prev_query or "information technology" in prev_query:
                    query = "Who is the HOD of the Information Technology department at RVR & JC College"
                elif "mechanical" in prev_query:
                    query = "Who is the HOD of the Mechanical Engineering department at RVR & JC College"
                elif "mca" in prev_query:
                    query = "Who is the HOD of the MCA department at RVR & JC College"
                elif "mba" in prev_query or "management sciences" in prev_query:
                    query = "Who is the HOD of the Management Sciences department at RVR & JC College"
                else:
                    query = "What are the departments at RVR & JC College"
            elif "lab" in prev_query or "laboratory" in prev_query:
                if "csd" in prev_query or "data science" in prev_query:
                    query = "What are the laboratory facilities in the Computer Science & Engineering (Data Science) department at RVR & JC College"
                elif "cse" in prev_query:
                    query = "What are the laboratory facilities in the Computer Science & Engineering department at RVR & JC College"
                elif "csbs" in prev_query or "machine learning" in prev_query:
                    query = "What are the laboratory facilities in the Computer Science and Machine Learning department at RVR & JC College"
                elif "chemical" in prev_query:
            ...
            if "swimming" in prev_query:
                query = "Is there a swimming facility at RVR & JC College"
            elif "cricket" in prev_query:
                query = "What are the cricket facilities at RVR & JC College"
            elif "gym" in prev_query:
                query = "What gym facilities are available at RVR & JC College"
